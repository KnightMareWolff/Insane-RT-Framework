...
uniform sampler2D   textura_cube_map;
...
{
    vec3 s = normalize(vec3(Luzes[IdLuz].posicao_luz - t_eyecoord));
    vec3 v = normalize(-t_eyecoord.xyz);
    vec3 r = reflect( -s, s_normal );
    float sDotN = max( dot(s,s_normal), 0.0 );
    vec3 ambient = Luzes[IdLuz].cor_ambiente_luz * cor_ambiente_mat;
    vec3 diffuse = Luzes[IdLuz].cor_diffusa_luz * cor_diffusa_mat * sDotN;
    vec3 spec = vec3(0.0);
    if( sDotN > 0.0 )
       spec = Luzes[IdLuz].cor_specular_luz * cor_specular_mat *
              pow( max( dot(r,v), 0.0 ), brilho_mat );
...
vec4 CalculaMaterial()
{
    ...
    if(modo_material==2)
    {
       ...
       //                   0.7);
       cor_material = (texture2D(textura_base, t_texcoord) - texture2D(textura_detalhe, t_texcoord * vec2(100.0,100.0)));
    }
    if(modo_material==3)
    {
    ...
    {
       //Realiza a Reflexão
       vec4 correflexao = texture2D(textura_cube_map, t_dir_reflexao);
    ...
    if(modo_material==5)
    {
       //Realiza a Refração
       vec4 correflexao = texture2D(textura_cube_map, t_dir_reflexao);
       vec4 correfracao = texture2D(textura_cube_map, t_dir_refracao);
